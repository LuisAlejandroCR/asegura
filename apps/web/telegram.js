...
// Every call here is version-gated and degrades to a no-op. The page is opened from WhatsApp,
// from Telegram and from a plain desktop browser, and it has to be equally good in all three:
// the Telegram chrome is an enhancement layered on top, never a dependency.

// Is this page running AS a Mini App? Telegram launches that view with tgWebApp* parameters —
// it is where the SDK itself reads initData from — and on desktop it also leaves a bridge on
// the window. Neither signal means there is nothing to switch on: not WhatsApp, not a plain
// browser, and not Telegram's own in-app browser, which is how the link arrives today.
const MARCADORES = /(^|[#&?])tgWebApp[A-Za-z]+=/;

function pareceMiniApp() {
  return MARCADORES.test(location.hash) || MARCADORES.test(location.search) ||
    Boolean(window.TelegramWebviewProxy);
}

// The SDK is fetched only in that case. Loading it for everyone would bill the majority — who
// arrive from WhatsApp — a blocking request to telegram.org for a feature they never see.
let cargando = null;
function cargarSdk() {
  ...
  if (!cargando) {
    cargando = new Promise((listo) => {
      ...
      el.src = 'https://telegram.org/js/telegram-web-app.js?63';
      el.onload = () => listo();
      // A failed fetch here must not leave the page half-built: it just carries on without.
      el.onerror = () => listo();
...
// Read lazily, never captured at import: the SDK now lands after this module is evaluated.
const tg = () => window.Telegram?.WebApp;

// Telegram's in-app browser also defines window.Telegram but draws no MainButton and reports
// platform "unknown". Trusting it would hide our own pay bar in favour of a button nobody can
// see — no way left to pay at all.
export function enMiniApp() {
  const w = tg();
  return Boolean(w && w.platform && w.platform !== 'unknown');
}
...
// Resolves true only when the page really is a Mini App, so the caller can switch layout to
// the client's stable viewport. Awaiting the SDK costs the Mini App path a few milliseconds
// before ready() hides Telegram's placeholder; it costs every other channel nothing at all.
export async function arrancar({ alCambiarTema, alEstabilizar } = {}) {
  if (!pareceMiniApp()) return false;
  await cargarSdk();
  const w = tg();
  if (!w) return false;

  w.ready();  // hides Telegram's placeholder at the first painted screen, not at last asset
  w.expand(); // full available height instead of a half sheet the person has to drag open
  ...
  if (desde('7.10')) w.setBottomBarColor('bottom_bar_bg_color');
  // The transcript scrolls. Without this, dragging it down closes the app mid-conversation.
  if (desde('7.7')) w.disableVerticalSwipes();
  ...
  w.onEvent('viewportChanged', (e) => { if (e?.isStateStable) alEstabilizar?.(); });
  if (desde('8.0')) {
    w.onEvent('safeAreaChanged', () => alEstabilizar?.());
    w.onEvent('contentSafeAreaChanged', () => alEstabilizar?.());
  }
  return true;
}
...
export const botonPrincipal = {
  ...
  ocultar() {
    if (!enMiniApp()) return;
    const b = tg().MainButton;
    if (alTocarPrincipal) { b.offClick(alTocarPrincipal); alTocarPrincipal = null; }
    b.hideProgress();
    b.hide();
  },
};
...
export const botonAtras = {
  mostrar(alTocar) {
    ...
    if (alTocarAtras) b.offClick(alTocarAtras);
  ...
  ocultar() {
    if (!desde('6.1')) return;
    const b = tg().BackButton;
    if (alTocarAtras) { b.offClick(alTocarAtras); alTocarAtras = null; }
    b.hide();
  },
};
