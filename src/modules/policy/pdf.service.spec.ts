...
describe('PdfService — petCount pricing display', () => {
  ...
  // The input is the final locked-in TOTAL, not a per-unit price: treating it as per-unit
  // multiplied it again, so a multi-pet certificate printed a total the customer never paid.
  // The per-pet line is derived by dividing back down.
...
describe('PdfService.resolveAuditUrl', () => {
  ...
  function resolveAuditUrl(policyId: string): string {
    return (service as any).resolveAuditUrl(policyId);
  }

  it('always resolves to the deterministic referenceURI', () => {
